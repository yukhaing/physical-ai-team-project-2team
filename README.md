...
OMX 로봇팔 적재 흐름으로 넘기는 통합 관제 프로토타입입니다. 현재 저장소의 중심은
불량 박스 이송 시나리오 검증이며, Beagle 실제 연동은 아직 구현되지 않았습니다.
...
- 작업 로그 SQLite 기록
- Beagle 상태를 가정한 시나리오 테스트

### Using Dummy/Temporary Values
- Beagle 실제 통신/API는 아직 미구현이며, 현재는 더미 상태 전이로 동작 테스트
