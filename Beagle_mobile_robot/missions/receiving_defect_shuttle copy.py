...
from the OMX arm, drives to the defect zone, waits for the operator's GUI
unload-complete signal, then drives back and waits again. All physical/tuning numbers (zone positions,
...
def next_state(state: str, data: ShuttleData, align_s: float) -> str:
    ...
    if state == "DWELL_DEFECT":
        return "GOTO_RECEIVING" if data.operator_unloaded else "DWELL_DEFECT"
...
class ShuttleMission:
    ...
    def step(self) -> dict[str, object]:
        ...
        data = ShuttleData(
            avoidance, goal_reached, self.box_signal.is_set(),
            self.box_signal.operator_unloaded(), align_elapsed)
        new_state = next_state(self.state, data, s.receiving_align_duration_s)
