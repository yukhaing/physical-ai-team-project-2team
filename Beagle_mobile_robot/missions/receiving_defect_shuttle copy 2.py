...
from common.localization import localize_robot
...
class MissionSettings:
    ...
    relocalize_search_radius_m: float
    relocalize_max_error_m: float
...
def load_settings(config_path: str | None = None) -> MissionSettings:
    ...
    settings = MissionSettings(
        ...
        relocalize_search_radius_m=mission_cfg.get("relocalize_search_radius_m", 0.15),
        relocalize_max_error_m=mission_cfg.get("relocalize_max_error_m", 0.05),
    )
...
class ShuttleMission:
    def __init__(
        ...
        *,
        ...
        self.receiving_target: tuple[float, float] = settings.receiving_m
    ...
    def _relocalize(self, label: str) -> None:
        """LiDAR scan-matching으로 dead-reckoned self.pose의 누적 오차를 절대 위치로 보정합니다.

        로봇이 zone에 도착해 정지한 직후에만 부른다 -- 움직이는 중에 스캔하면
        매칭 자체가 왜곡된다. 매칭오차(err_m)가 relocalize_max_error_m를 넘으면
        (박스 등 장애물이 벽을 가린 경우 등) 신뢰하지 않고 dead-reckoned pose를 유지한다.
        """
        self.robot.wheels(0.0, 0.0)
        s = self.settings
        try:
            pose_est, err_m = localize_robot(
                self.robot, self.segments, self.pose.x, self.pose.y,
                search_radius=s.relocalize_search_radius_m, verbose=False,
            )
        except ValueError:
            print(f"[relocalize] {label}: no valid lidar points, skipped")
            return
        if err_m > s.relocalize_max_error_m:
            print(f"[relocalize] {label}: match too poor (err={err_m * 1000:.1f}mm), keeping dead-reckoned pose")
            return
        dx_cm = (pose_est.x - self.pose.x) * 100.0
        dy_cm = (pose_est.y - self.pose.y) * 100.0
        dtheta_deg = math.degrees(wrap_angle(pose_est.theta - self.pose.theta))
        print(f"[relocalize] {label}: corrected by ({dx_cm:+.1f},{dy_cm:+.1f})cm, {dtheta_deg:+.1f}deg (err={err_m * 1000:.1f}mm)")
        self.pose = pose_est

    def start(self) -> None:
        self._gyro_bias = calibrate_gyro_bias(self.robot)
        self._relocalize("mission_start")
        # A human only ever places the robot once, at mission start -- lock that
        # scan-matched position in as the real receiving-zone target so every
        # later GOTO_RECEIVING returns to where the robot actually began instead
        # of course_config.json's nominal (and possibly slightly-off) coordinate.
        self.receiving_target = (self.pose.x, self.pose.y)
...
def run_mission(
    ...
    *,
    ...
    mission = ShuttleMission(robot, box_signal, settings, segments, status_client=status_client)
    mission.start()
...
def main() -> None:
    ...
    try:
        with SafeBeagle(dry_run=args.dry_run, scene=args.scene, max_speed=settings.nav_max_percent) as robot:
            ...
            result = run_mission(robot, box_signal, settings, segments, duration_s=args.duration, cycles=args.cycles, output_path=args.output, visualizer=visualizer, status_client=status_client)
