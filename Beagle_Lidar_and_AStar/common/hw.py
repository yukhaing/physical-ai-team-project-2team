...
class MissionInterrupted(RuntimeError):
    """Raised after wheel output is stopped by the GUI safety callback."""
...
class Hardware:
    ...
    def __init__(
        self,
        max_wheel_percent: float = DEFAULT_MAX_WHEEL_PERCENT,
        *,
    ...
    def _check_interrupted(self) -> None:
        if self.should_stop is not None and self.should_stop():
            self.stop()
            raise MissionInterrupted("mission interrupted by safety callback")
