...
AUTOMATIC_UNLOAD_OMX="${AUTOMATIC_UNLOAD_OMX:-true}"
ENABLE_UNLOAD_OMX="${ENABLE_UNLOAD_OMX:-true}"
...
UNLOAD_VIDEO_DEVICE="${UNLOAD_VIDEO_DEVICE:-/dev/video2}"
...
source_ros() {
  ...
  export OMX_VIDEO_DEVICE VIDEO_DEVICE UNLOAD_VIDEO_DEVICE
}

ros_command() {
  local command="$1"
  printf 'source /opt/ros/jazzy/setup.bash; source /root/ros2_ws/install/setup.bash; source %q; export PATH=/opt/ultralytics-venv/bin:$PATH; export YOLO_CONFIG_DIR=/tmp/Ultralytics; export BEAGLE_MODE=%q BEAGLE_TRIGGER_HOST=%q BEAGLE_TRIGGER_PORT=%q BEAGLE_STATUS_PORT=%q BEAGLE_LOCAL_MISSION_LAUNCH=%q BEAGLE_LOCAL_PORT_NAME=%q AUTOMATIC_UNLOAD_OMX=%q OMX_VIDEO_DEVICE=%q UNLOAD_VIDEO_DEVICE=%q; cd %q; exec %s' \
    ...
    "${AUTOMATIC_UNLOAD_OMX}" "${VIDEO_DEVICE}" "${UNLOAD_VIDEO_DEVICE}" \
    "${GUI_WORKSPACE}" "${command}"
}
...
wait_for_service() {
  local service="$1" seconds="$2" description="$3"
  local deadline=$((SECONDS + seconds))
  while (( SECONDS < deadline )); do
    if ros2 service list 2>/dev/null | grep -Fxq "${service}"; then
      echo "Ready: ${description} (${service})"
      return 0
    fi
    sleep 1
  done
  echo "ERROR: ${description} did not become ready: ${service}" >&2
  return 1
}

initialize_unload_omx() {
  local reset_service="/unload_omx/unload_coordinator/reset"
  local status_topic="/unload_omx/unload_coordinator/status"
  local response message

  wait_for_message /unload_omx/joint_states 15 "unloading OMX feedback"
  wait_for_service "${reset_service}" 15 "unloading OMX reset service"
  response="$(
    timeout 10 ros2 service call \
      "${reset_service}" std_srvs/srv/Trigger '{}' 2>&1
  )" || {
    echo "ERROR: unloading OMX startup reset call failed." >&2
    echo "${response}" >&2
    return 1
  }
  if [[ "${response}" != *"success=True"* &&
        "${response}" != *"success: true"* ]]; then
    echo "ERROR: unloading OMX rejected startup reset." >&2
    echo "${response}" >&2
    return 1
  fi
  echo "Initializing unloading OMX: reset accepted; waiting for parking and gripper close."

  # Keep one subscription alive so the short final-close completion message
  # cannot fall into a gap between repeated `--once` subscriptions.
  message="$(
    timeout 50 ros2 topic echo "${status_topic}" std_msgs/msg/String 2>/dev/null |
      grep -m1 -E \
        'IDLE: unload OMX reset complete; parking ready|FAILED:'
  )" || true
  if [[ "${message}" == *"IDLE: unload OMX reset complete; parking ready"* ]]; then
    echo "Ready: unloading OMX initialized at parking with gripper closed."
    return 0
  fi
  if [[ "${message}" == *"FAILED:"* ]]; then
    echo "ERROR: unloading OMX startup reset failed." >&2
    echo "${message}" >&2
    return 1
  fi
  echo "ERROR: unloading OMX startup reset did not complete within 50 seconds." >&2
  return 1
}
...
start_system() {
  ...
  [[ -e "${UNLOAD_VIDEO_DEVICE}" ]] || { echo "ERROR: unloading camera device not found: ${UNLOAD_VIDEO_DEVICE}" >&2; exit 1; }
  if [[ "$(readlink -f "${VIDEO_DEVICE}")" == "$(readlink -f "${UNLOAD_VIDEO_DEVICE}")" ]]; then
    echo "ERROR: loading and unloading cameras resolve to the same device." >&2
    exit 1
  fi
  if [[ "${BEAGLE_MODE}" == "local" && "${BEAGLE_LOCAL_MISSION_ENABLED}" == "true" ]]; then
    for serial_device in "${PORT_NAME}" "${UNLOAD_PORT_NAME}"; do
      if [[ -n "${serial_device}" &&
            "$(readlink -f "${serial_device}")" == "$(readlink -f "${BEAGLE_LOCAL_PORT_NAME}")" ]]; then
        echo "ERROR: Beagle receiver and OMX resolve to the same serial device." >&2
        exit 1
      fi
    done
    [[ -x "${BEAGLE_PYTHON}" ]] || {
  ...
  if [[ -n "${UNLOAD_PORT_NAME}" ]]; then
    new_window unload_omx "ros2 launch omx_box_control unload_omx_system.launch.py port_name:=${UNLOAD_PORT_NAME} start_zenoh:=false"
    wait_for_node /unload_omx/unload_coordinator 35 "unloading OMX coordinator"
    initialize_unload_omx
  fi
  new_window camera_load "ros2 launch open_manipulator_bringup camera_usb_cam.launch.py name:=camera_devices/loading video_device:=${VIDEO_DEVICE}"
  new_window camera_unload "ros2 launch open_manipulator_bringup camera_usb_cam.launch.py name:=camera_devices/unloading video_device:=${UNLOAD_VIDEO_DEVICE}"
  wait_for_message /camera_devices/loading/image_raw 20 "loading USB camera"
  wait_for_message /camera_devices/unloading/image_raw 20 "unloading USB camera"
  ...
  echo
  echo "OMX GUI system is ready. The unloading OMX was initialized automatically."
  echo "No pick cycle has been requested."
...
status_system() {
  ...
  for topic in /joint_states /camera/image_raw /unload_camera/image_raw /console/annotated_image /unload_vision/annotated_image /camera_roles/status /console/status; do
