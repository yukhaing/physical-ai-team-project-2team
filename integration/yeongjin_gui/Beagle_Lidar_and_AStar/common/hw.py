...
class Hardware:
    ...
    def __init__(self, max_wheel_percent: float = DEFAULT_MAX_WHEEL_PERCENT,
                 port_name: str | None = None,
                 control_poll: Callable[[], None] | None = None) -> None:
        from roboid import Beagle  # type: ignore

        self.robot: Any = Beagle(0, port_name) if port_name else Beagle()
    ...
    def _poll_control(self) -> None:
        if self.control_poll is not None:
            self.control_poll()
    ...
    def stop(self) -> None:
        try:
            ...
            # A disappearing receiver must not mask the mission's original
            # failure or prevent status/log cleanup.
            pass
