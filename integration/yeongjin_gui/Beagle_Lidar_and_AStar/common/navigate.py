...
# A low map-match residual alone is not sufficient in this rectangular room:
# a scan can fit a wrong, symmetric pose surprisingly well. Periodic
# localization is only meant to trim small drift since the last checkpoint,
# so reject corrections inconsistent with the encoder/gyro pose.
LOCALIZE_MAX_MATCH_ERR_M = 0.050
# Match the configured 8cm local search envelope: legitimate translational
# drift may use most of it, while the observed bad fixes still fail on their
# 10cm+ displacement and 25deg+ heading innovation.
LOCALIZE_MAX_POSITION_JUMP_M = 0.080
LOCALIZE_MAX_HEADING_JUMP_RAD = math.radians(12.0)


def localization_correction_is_plausible(
    odom_pose: Pose2D,
    localized_pose: Pose2D,
    match_err_m: float,
    max_match_err_m: float = LOCALIZE_MAX_MATCH_ERR_M,
    max_position_jump_m: float = LOCALIZE_MAX_POSITION_JUMP_M,
    max_heading_jump_rad: float = LOCALIZE_MAX_HEADING_JUMP_RAD,
) -> bool:
    """Return whether a periodic map correction is safe to apply."""
    position_jump_m = odom_pose.distance_to(localized_pose.x, localized_pose.y)
    heading_jump_rad = abs(wrap_angle(localized_pose.theta - odom_pose.theta))
    return (
        math.isfinite(match_err_m)
        and match_err_m <= max_match_err_m
        and position_jump_m <= max_position_jump_m
        and heading_jump_rad <= max_heading_jump_rad
    )
    ...
    try:
        while time.monotonic() - start_time < max_time_s:
            ...
            if due_checkpoint:
                ...
                plausible = localization_correction_is_plausible(pose, localized, match_err)
                ...
                if plausible:
                    ...
                    print("  [localize] REJECTED unsafe correction; keeping encoder/gyro odometry "
                          f"(limits: {LOCALIZE_MAX_MATCH_ERR_M * 1000:.0f}mm match, "
                          f"{LOCALIZE_MAX_POSITION_JUMP_M * 100:.1f}cm position, "
                          f"{math.degrees(LOCALIZE_MAX_HEADING_JUMP_RAD):.0f}deg heading)")
                previous = time.monotonic()  # exclude the stopped checkpoint time from the next dt
...
def goto_zone(
    ...
    if align_map:
        ...
        converged = find_pose_via_map(
            ...
            initial_heading_rad=arrival_pose.theta, initial_pose=arrival_pose,
        )
        print("RESULT:", f"arrived and converged at {to_name}" if converged
              else f"drove to {to_name} but did NOT fully converge there -- see log above")
        return converged
