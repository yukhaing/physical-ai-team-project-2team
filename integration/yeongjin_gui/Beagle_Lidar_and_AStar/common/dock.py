...
# Final alignment can otherwise turn/drive from a wrong but low-residual map
# match. At this point arrival odometry is a strong local prior; a result far
# from it must fail closed (no wheel command) rather than be acted on.
MAP_MAX_MATCH_ERR_M = 0.050
MAP_MAX_POSITION_INNOVATION_M = 0.080
MAP_MAX_HEADING_INNOVATION_RAD = math.radians(20.0)
MAP_SEARCH_EDGE_MARGIN_M = 0.010


def map_pose_is_plausible(
    prior: Pose2D,
    localized: Pose2D,
    match_err_m: float,
    search_radius_m: float,
    max_match_err_m: float = MAP_MAX_MATCH_ERR_M,
    max_position_innovation_m: float = MAP_MAX_POSITION_INNOVATION_M,
    max_heading_innovation_rad: float = MAP_MAX_HEADING_INNOVATION_RAD,
) -> bool:
    """Reject ambiguous final-align matches before any physical movement."""
    dx = localized.x - prior.x
    dy = localized.y - prior.y
    at_search_edge = (
        abs(dx) >= max(0.0, search_radius_m - MAP_SEARCH_EDGE_MARGIN_M)
        or abs(dy) >= max(0.0, search_radius_m - MAP_SEARCH_EDGE_MARGIN_M)
    )
    return (
        math.isfinite(match_err_m)
        and match_err_m <= max_match_err_m
        and math.hypot(dx, dy) <= max_position_innovation_m
        and abs(wrap_angle(localized.theta - prior.theta)) <= max_heading_innovation_rad
        and not at_search_edge
    )
...
def find_pose_via_map(
    ...
    if heading_tol_rad is None:
        heading_tol_rad = math.radians(REALIGN_TOL_DEG)
    guess_x = initial_pose.x if initial_pose is not None else target_x
    guess_y = initial_pose.y if initial_pose is not None else target_y
    ...
    if initial_pose is not None:
        guess_theta = initial_pose.theta
        heading_range_rad = math.radians(heading_window_deg)
    elif initial_heading_rad is None:
    ...
    for i in range(max_iters):
        ...
        prior = Pose2D(guess_x, guess_y, guess_theta)
        # Only enforce innovation against a real arrival pose. Standalone
        # cold-start tooling has no trustworthy pose prior, so it still uses
        # match quality and the search-edge guard without pretending the
        # configured target is a measured current pose.
        max_position_innovation_m = MAP_MAX_POSITION_INNOVATION_M if initial_pose is not None else math.inf
        max_heading_innovation_rad = MAP_MAX_HEADING_INNOVATION_RAD if initial_pose is not None else math.pi
        if not map_pose_is_plausible(
            prior, localized, match_err, search_radius_m,
            max_position_innovation_m=max_position_innovation_m,
            max_heading_innovation_rad=max_heading_innovation_rad,
        ):
            pos_jump_m = prior.distance_to(localized.x, localized.y)
            heading_jump_deg = math.degrees(wrap_angle(localized.theta - prior.theta))
            print(f"[pose-map {i}] REJECTED unsafe localization before movement: "
                  f"match_err={match_err * 1000:.0f}mm, position_jump={pos_jump_m * 100:.1f}cm, "
                  f"heading_jump={heading_jump_deg:+.1f}deg. Robot remains stopped.")
            hw.stop()
            return False
