...
카메라는 Z를 결정하지 않는다. 현재 접근 Z는 `0.19253m`, 파지 Z는 `0.14099m`다.
트레이 또는 OMX 베이스 설치가 달라졌다면 빈 그리퍼로 이 높이를 먼저 검증한다.
...
3. 수직 하강해 박스를 집고 같은 경로로 상승한다. 빈 파지이면 상승 후 최대 2회
   다시 내려가 집기를 재시도한다.
4. 박스를 안전 높이로 올린 시점에 관제가 Beagle로 `box_picked`를 보내며, Beagle은
   수령 위치 복귀를 시작한다.
5. Beagle 복귀와 병렬로 관절 1을 180도 회전해 뒤쪽 배출 위치로 이동한다.
6. 수직 하강해 놓고 상승한 뒤 빈 그리퍼를 원래 방향으로 돌린다.
7. 높은 staging을 거쳐 카메라 시야를 비우는 vision parking으로 복귀하고 그리퍼를
   닫은 뒤 `COMPLETE`를 발행한다.

재시도를 모두 실패하면 하역 OMX는 빈 그리퍼를 올려 parking으로 복귀하고 Beagle도
수령 위치로 돌려보낸다. 박스를 올리기 전의 다른 오류는 Beagle을 하역 위치에 정지시킨다.
`box_picked` 이후의 오류는 표시만 하고 복귀 신호를 중복 전송하지 않는다.
