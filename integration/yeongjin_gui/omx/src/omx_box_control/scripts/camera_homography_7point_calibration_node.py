...
class CameraHomography7PointCalibration(Node):
    ...
    def load_existing_calibration(self):
        """Load saved click points for visual inspection without changing them."""
        if not os.path.isfile(self.output_file):
            return
        storage = cv2.FileStorage(self.output_file, cv2.FILE_STORAGE_READ)
        if not storage.isOpened():
            self.get_logger().warning(
                f'Cannot open existing calibration: {self.output_file}')
            return
        matrix = storage.getNode('homography').mat()
        image = storage.getNode('image_points').mat()
        errors = storage.getNode('reprojection_errors_mm').mat()
        storage.release()
        if (matrix is None or matrix.shape != (3, 3) or image is None or
                image.size != 14 or not np.isfinite(matrix).all() or
                not np.isfinite(image).all()):
            self.get_logger().warning(
                f'Existing calibration is incomplete: {self.output_file}')
            return
        self.homography = matrix.astype(np.float64)
        self.image_points = [tuple(point) for point in
                             image.reshape((7, 2)).astype(float)]
        if (errors is not None and errors.size == 7 and np.isfinite(errors).all()):
            self.errors_mm = errors.reshape(7).astype(float)
        else:
            projected = cv2.perspectiveTransform(
                np.asarray(self.image_points, dtype=np.float64).reshape((-1, 1, 2)),
                self.homography).reshape((-1, 2))
            self.errors_mm = np.linalg.norm(
                projected - self.reference_points, axis=1) * 1000.0
        self.get_logger().info(
            f'Loaded existing 7-point calibration for display: {self.output_file}')
    ...
    def display_image(self):
        ...
        for index, (x, y) in enumerate(self.image_points):
            ...
            reference = self.reference_points[index]
            cv2.putText(
                display, f'P{index + 1} ({int(x)},{int(y)})',
                (int(x) + 8, int(y) - 10), cv2.FONT_HERSHEY_SIMPLEX,
                0.48, (0, 255, 255), 2)
            cv2.putText(
                display, f'X={reference[0]:.3f} Y={reference[1]:.3f}',
                (int(x) + 8, int(y) + 10), cv2.FONT_HERSHEY_SIMPLEX,
                0.42, (0, 255, 255), 1)
