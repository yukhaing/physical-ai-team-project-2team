...
class PickCoordinator(Node):
    ...
    def __init__(self):
        ...
        defaults = {
            ...
            # Integrated automatic operation can verify the original camera
            # location after the arm has moved the alleged load away.  A
            # rejected grasp returns from the high place approach to staging
            # without descending over Beagle.
            'allow_post_transfer_grasp_reject': False,
        ...
        self.create_service(Trigger, '~/reject_grasp', self.on_reject_grasp)
    ...
    def on_reject_grasp(self, _request, response):
        """Abort a visually empty grasp before place descent or Beagle dispatch."""
        if not bool(self.p('allow_post_transfer_grasp_reject')):
            response.success, response.message = False, (
                'post-transfer grasp rejection is disabled')
            return response
        if self.state != self.WAIT_BEAGLE_ARRIVAL:
            response.success, response.message = False, (
                f'grasp rejection is not valid in {self.state}')
            return response
        reason = 'pick failed: defect remained at the original pick location'
        self.empty_grasp_recovery = True
        self.empty_grasp_failure_reason = reason
        response.success, response.message = self.request(
            'staging', self.WAIT_RETURN_STAGING,
            'post-transfer empty-grasp safe return staging')
        return response
