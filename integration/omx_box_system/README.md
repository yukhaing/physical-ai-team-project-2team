...
잘못 클릭했으면 `u`로 마지막 점을 취소하고, 전부 다시 하려면 `c`, 초기화는
`r`, 종료는 `q`를 사용한다. 일곱 번째 점을 클릭하면 다음 파일로 자동 저장된다.

```text
integration/omx_box_system/calibration/omx_camera_homography_7point.yaml
```

화면과 로그의 평균·최대 재투영 오차를 확인한다. 저장 후 화면의 별도 지점을
클릭하면 변환된 `link0 X/Y`가 자홍색으로 표시되고 로그에도 출력된다. 이 확인
클릭은 로봇 target을 발행하지 않는다. `v`를 누르면 확인 표시만 지울 수 있다.
재투영 오차는 사용한 기준점에 대한 오차이므로, calibration에 쓰지 않은 별도
위치들을 실측해 최종 XY 오차를 검증해야 한다. 카메라 위치나 각도, 작업면 높이가 바뀌면 다시
